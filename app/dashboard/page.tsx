...
export default function Dashboard() {
  ...
  async function calculateMetrics() {
    ...
    const { data: propData } = await supabase.from('properties').select('price, construction_value')
    const { data: expData } = await supabase.from('expenses').select('amount').gte('date', startDate).lte('date', endDate)

    if (propData) {
      setTotalRevenue(propData.reduce((acc, curr) => acc + (curr.price || 0), 0) * 12)
      setTotalAmortization(propData.reduce((acc, curr) => acc + (curr.construction_value || 0), 0) * 0.03)
    }
    if (expData) setTotalExpenses(expData.reduce((acc, curr) => acc + (curr.amount || 0), 0))
    
    const today = new Date().toISOString()
    const limit = new Date(Date.now() + 15 * 24 * 60 * 60 * 1000).toISOString()
    const { data: expAlerts } = await supabase.from('expenses').select('category, expiry_date, properties(name)').gte('expiry_date', today).lte('expiry_date', limit)
    if (expAlerts) setAlerts(expAlerts)
    
    setRefreshKey(prev => prev + 1)
  }

  useEffect(() => { calculateMetrics() }, [selectedYear])
    ...
    <div className="p-4 sm:p-8 bg-slate-50 min-h-screen w-full max-w-[100vw] overflow-x-hidden">
        ...
        <div className="flex flex-wrap gap-2 items-center bg-white p-2 rounded-xl border shadow-sm">
          <div className="flex items-center gap-2 mr-4 border-r pr-4">
            ...
            <Select value={selectedYear} onValueChange={setSelectedYear}>
              <SelectTrigger className="w-[100px] h-8 bg-slate-50 border-none font-black text-blue-700"><SelectValue /></SelectTrigger>
              <SelectContent>
      ...
      {alerts.length > 0 && (
        <div className="mb-8 p-4 bg-red-50 border-l-4 border-red-500 rounded-r-lg text-red-800 shadow-sm flex items-start gap-3">
          ...
          <div>
            ...
            {alerts.map((a, i) => (<p key={i} className="text-sm">{a.category} en <strong>{a.properties.name}</strong> ({a.expiry_date})</p>))}
          </div>
        </div>
      )}
      ...
      <Tabs defaultValue="finance" className="w-full">
        <TabsList className="grid w-full max-w-[400px] grid-cols-2 mb-8 bg-slate-200/50 p-1 rounded-lg">
          <TabsTrigger value="finance" className="data-[state=active]:bg-white data-[state=active]:shadow-sm font-bold flex gap-2">
            <Building2 className="w-4 h-4" /> Finanzas y Activos
          </TabsTrigger>
          <TabsTrigger value="tenants" className="data-[state=active]:bg-white data-[state=active]:shadow-sm font-bold flex gap-2">
        ...
        <TabsContent value="finance" className="space-y-8 animate-in fade-in duration-500">
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
            <Card className="border-l-4 border-l-blue-600 shadow-sm"><CardHeader className="pb-1 text-[10px] text-slate-400 font-bold uppercase tracking-widest">Ingresos Brutos Anuales</CardHeader>
              <CardContent><div className="text-2xl font-black">{totalRevenue.toLocaleString('es-ES')} €</div></CardContent></Card>
            <Card className="border-l-4 border-l-red-500 shadow-sm"><CardHeader className="pb-1 text-[10px] text-slate-400 font-bold uppercase tracking-widest">Gastos Deducibles</CardHeader>
              <CardContent><div className="text-2xl font-black text-red-600">-{totalExpenses.toLocaleString('es-ES')} €</div></CardContent></Card>
            <Card className="border-l-4 border-l-emerald-500 shadow-sm"><CardHeader className="pb-1 text-[10px] text-slate-400 font-bold uppercase tracking-widest">Ahorro Amortización</CardHeader>
              <CardContent><div className="text-2xl font-black text-emerald-600">-{totalAmortization.toLocaleString('es-ES')} €</div></CardContent></Card>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
            <div className="xl:col-span-2 space-y-8">
              <div className="bg-white p-6 rounded-xl border shadow-sm">
                <h2 className="text-lg font-bold text-slate-800 mb-4 flex items-center gap-2">🏠 Salud de Activos</h2>
                <PropertyList key={`props-${refreshKey}`} />
              </div>
              <div className="bg-white p-6 rounded-xl border shadow-sm">
                <h2 className="text-lg font-bold text-slate-800 mb-4 flex items-center gap-2"><TrendingUp className="w-5 h-5 text-blue-600" /> Rendimiento</h2>
            ...
            <div className="space-y-8">
              <div className="p-6 bg-amber-50 rounded-xl border border-amber-200 shadow-sm">
                <h2 className="text-sm font-black text-amber-800 uppercase mb-4 tracking-widest flex items-center gap-2">🔨 Mantenimiento</h2>
              ...
              <Card className="bg-blue-600 text-white shadow-lg">
                ...
                <CardContent className="space-y-4 text-blue-100">
                  ...
                  <div className="pt-4 border-t border-blue-500/50">
                    <p className="text-xs font-bold uppercase tracking-widest mb-4 text-blue-200">Acciones Rápidas</p>
                    <AddTenantForm />
                  </div>
                </CardContent>
              </Card>
