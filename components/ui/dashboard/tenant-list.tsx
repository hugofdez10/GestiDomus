              ...
              <TableRow key={t.id} className="hover:bg-slate-50">
                ...
                <TableCell className="text-right">
                  <div className="flex justify-end items-center gap-2">
                    {t.document_url && (
                      <a href={t.document_url} target="_blank" rel="noopener noreferrer" className="p-2 hover:bg-blue-100 rounded-full transition-colors text-blue-600">
                        <FileText className="w-4 h-4" />
                      </a>
                    )}
                    <EditTenantForm tenant={t} onUpdate={fetchTenants} />
                    <button onClick={() => handleDelete(t.id, t.full_name)} className="p-2 hover:bg-red-50 rounded-full transition-colors text-slate-400 hover:text-red-600">
                      <Trash2 className="w-4 h-4" />
                    </button>
                    <button onClick={() => openWhatsApp(t.phone, t.full_name, t.properties?.name)} className="inline-flex items-center justify-center p-2 bg-[#25D366] hover:bg-[#128C7E] text-white rounded-full transition-colors shadow-sm">
